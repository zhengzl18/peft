...
def preprocess_xxx(
    ...
    if sloppy_basis_path is not None and os.path.exists(sloppy_basis_path) and os.listdir(sloppy_basis_path):
        ...
        for name, module in target_modules(model, xxx_config):
            ...
            if not os.path.exists(f"{sloppy_basis_path}/{file_name}.pt"):
                ...
                sloppy_basis = {k: v.to(model.dtype) for k, v in sloppy_basis.items()}
    ...
    if not use_cache:
        ...
        calculate_jacobian(model, knowledge_loader, xxx_config, jacobian_path)

        # Calculate eigens
        # calculate_eigens(model, xxx_config, jacobian_path, eigen_path)

        # Crop CorDA eigens so that there's less to save
        calculate_sloppy_basis(model, xxx_config, jacobian_path, sloppy_basis_path, eigen_path=eigen_path, target_jacobian_path="/Data2/zhengzhilong/jacobian/MetaMATH_facebook_opt-125m_128_233")
...
def calculate_jacobian(
    ...
    for name, p in target_params(model, config):
        if os.path.exists(f"{jacobian_path}/{name.replace('.', '-')}.pt"):
        ...
        for data in tqdm(knowledge_loader):
            ...
            grads.append(p.grad.reshape(-1).clone().cpu())
        model.zero_grad()
        
        # stack grads into jacobian matrix
        jac = torch.stack(grads, dim=0)
        assert '-' not in name
        torch.save(jac, f"{jacobian_path}/{name.replace('.', '-')}.pt")
